...
def get_triplet_pairs(
        ...
        if unique_sequences.size > 1:
            for i, sequence_1 in enumerate(unique_sequences):
                for sequence_2 in unique_sequences[i + 1 :]:
                    ...
                    if USE_LAST_SAMPLE_ONLY:
                        triplet_ids.append(ids_1[-1], ids_2[-1])
                    elif ALWAYS_AGAINST_LAST:
                        for id_1 in ids_1:
                            triplet_ids.append([id_1, ids_2[-1]])

                        for id_2 in ids_2:
                            triplet_ids.append([ids_1[-1], id_2])
                    else:
                        for id_1 in ids_1:
                            for id_2 in ids_2:
                                triplet_ids.append([id_1, id_2])

    n_positives = len(triplet_ids)
    ...
    n_negatives = 0
    while n_negatives < n_positives:
        id_1 = triplet_ids[n_negatives][0]
        id_2 = random.sample(ids, 1)[0]
        ...
        dist = np.linalg.norm(centroids[id_1] - centroids[id_2])
        if dist >= MIN_DISTANCE_NEGATIVES:
            triplet_ids[n_negatives].append(id_2)
            n_negatives += 1
